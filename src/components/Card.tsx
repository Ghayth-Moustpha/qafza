...
const Card: React.FC<CardProps> = ({ title, body, index ,image}) => {
  ...
  return (
    <div
      ref={cardRef}
      className={`sticky top-40 h-500 bg-white shadow-xl border  z-${index + 1} transition-transform duration-500 ease-in-out`}
      style={{
        transform: isVisible
    ...
    >
      ...
      <img src={image} alt="imagecard" className='h-5 mx-2 md:h-10'/>
        ...
        <h2 className=" md:text-2xl font-bold text-purpleRoyal">
          {title} 
        </h2>
      </div>
